  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        {[...Array(3)].map((_, index) => (
          <div key={index} className="h-64 rounded-2xl border border-white/5 bg-surface-2 animate-pulse" />
        ))}
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 border-b border-white/5 pb-6 md:flex-row md:items-center md:justify-between">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <span className="rounded bg-white/5 px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-text-3">
              {summaryLabel}
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex rounded-lg border border-white/5 bg-surface-2 p-1">
            <SortButton
      ...
      {tickets.length > 0 ? (
        ...
        <div className="flex flex-col items-center py-20 text-center">
          <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full border border-white/5 bg-surface-2">
      ...
      {pagination.totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-white/5 pt-6">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-white/8 px-4 py-2 text-[11px] font-bold uppercase tracking-[0.18em] text-text-2 transition-colors hover:border-white/15 hover:text-text-1 disabled:cursor-not-allowed disabled:opacity-40"
          >
            <ChevronLeft size={14} />
            Anterior
          </button>
